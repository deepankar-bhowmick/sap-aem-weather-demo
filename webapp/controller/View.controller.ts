...
export default class View extends Controller {
    weather: SolaceConnection.WEATHER[] = []
            ...
            this.session.on(solace.SessionEventCode.CONNECT_FAILED_ERROR, (error: solace.OperationError) => {
                ...
                .on(solace.SessionEventCode.MESSAGE, (message: solace.Message) => {
                    this.getWeather(message)
                        .then((weather: SolaceConnection.WEATHER) => {
            ...
            this.session.on(solace.SessionEventCode.CONNECT_FAILED_ERROR, (error: solace.OperationError) => {
                ...
                .on(solace.SessionEventCode.UP_NOTICE, () => {
                    ...
                    let weatherConsumer = this.session.createMessageConsumer({
                        ...
                        .on(solace.MessageConsumerEventName.MESSAGE, (message: solace.Message) => {
                            const weather: SolaceConnection.WEATHER = JSON.parse(message.getBinaryAttachment() as string)
                            this.weather.forEach((weather_: SolaceConnection.WEATHER) => {
    ...
    private getWeather(message: solace.Message): Promise<SolaceConnection.WEATHER> {
        const coordinate: SolaceConnection.WEATHER = JSON.parse(message.getBinaryAttachment() as string)
            ...
            buttons: [new Button({
                ...
                press: (event: Event) => {
                    if ((this.byId('weather')?.getModel('local') as JSONModel) != null) {
                        this.weather = [];
                        (this.byId('weather')?.getModel('local') as JSONModel).setData(this.weather);
